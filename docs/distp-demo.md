...
# Distribution-based Business cases

We can make useful business cases using this approach. However, it's not a real Gibbs sampler/ Markov-chain Monte Carlos experiment... simply a Monte Carlos.

We certaintly could performa a MCMC approach. However, it's not really worth the complexity it would create. _Why?_

1. We have insufficient information to set an appropriate prior. _Apriori_, we don’t know the true distribution of our parameters.
2. Where (1) isn't true e.g., where we have data to measure, we don’t know the conditional effect (i.e., paired variable covariance) on the distribution from the most critical variables, namely engagement and the treatment effect. Thus, it’s likely better to assume absolute ignorance rather than create a partially broken conditional probability chain. This mimicks typical business cases (which obviously don’t use markov-chain-based distribution sampling methods).
3. Most business cases do not have complex bayesian priors and probabilities are, in general, independant i.e., ${tex`P(a|b)=P(a)\cdot P(b)`}. Bayesian evaluation is great for understanding the conditional probability. However, there are nuances, which we'll go into.
